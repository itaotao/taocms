...
class DbAction extends CommonAction{
	...
	public function index(){
     ...
     $this->assign("list", json_encode($tabs));
        $this->display();
	}
	public function import(){
		foreach (glob(C("DB_BACKUP")."*.sql") as $filename) {
    $arr[]=array("filename"=>urlchange($filename),"size"=>byteFormat(filesize($filename)));
 }
	...
	public function optimize(){
		...
		if(!empty($table)){
		$M = M();
         $result = $M->query('OPTIMIZE TABLE '.$table);
	...
	public function repair(){
		...
		if(!empty($table)){
		$M = M();
         $result = $M->query('REPAIR TABLE '.$table);
